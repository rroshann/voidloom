...
/// The free pan/zoom canvas presentation. Extracted verbatim from the original
/// `RootView` so Canvas behavior is unchanged; `RootView` now switches between
/// this and `SpacesShellView` on the persisted `app.mode` flag.
struct CanvasShellView: View {
...
/// Carries the floating dock's measured height up to `CanvasShellView` so grid
